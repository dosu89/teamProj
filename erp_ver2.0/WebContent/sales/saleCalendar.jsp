...
<link rel="stylesheet" type="text/css" href="/erp_ver2.0/css/saleCalendar.css">
		...
		$.ajax({
			...
			success: function(res){
				...
				for (let i = 0; i < jo.length; i++) {
					htmlString += 
					'<tr><td class="dialog_tbl">'+ (i+1) +
						'</td><td class="dialog_tbl"><span>' + 
							jo[i].pr_name + '</span></td><td>' 
						+ jo[i].amount + '</td>' + 
						'<td class="dialog_tbl">' + jo[i].sales + '</td><tr>' 
				}
